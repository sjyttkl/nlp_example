...
def create_dico(item_list):
    """
    Create a dictionary of items from a list of list of items.,统计词频
    """
...
def iob2(tags):
    ...
    for i, tag in enumerate(tags):
        ...
        if split[0] == 'B':
            ...
            tags[i] = 'B' + tag[1:]
        elif tags[i - 1][1:] == tag[1:]:  # 前后要一致  B-ORG ， I-ORG ，这里的ORG要一样的
            continue
...
class BatchManager(object):
    ...
    def __init__(self, data, batch_size, max_length=None):
    ...
    def sort_and_pad(self, data, batch_size, max_length):
        ...
        for i in range(num_batch):
            batch_data.append(
                self.pad_data(sorted_data[i * batch_size: (i + 1) * batch_size], max_length=max_length))
        return batch_data
    ...
    def iter_batch(self, shuffle=False):
        ...
        return self.batch_data[0:self.len_data]
        # for idx in range(self.len_data):
        #     yield self.batch_data[idx]

    def get_batch(self, batch_data, batch_size):
        inputs_seq_batch = []
        inputs_seq_len_batch = []
        outputs_seq_batch = []
        for data in batch_data[:]:
            inputs_seq_batch.extend(data[1])
            inputs_seq_len_batch.extend([[np.sum(np.sign(x))] for x in data[1]])
            outputs_seq_batch.extend(data[3])
